...
class PROC(Feature):
    """
    The returns of the exponential weighted price
    """
    ...
    def __init__(
        ...
        super().__init__(name, update_frequency, lookback_periods+1, normalisation_on)
...
class RSI(Feature):
    ...
    def _update(self, state: State) -> None:
        self.closes.append(state.market.ewm)
        if len(self.closes) < self.lookback_periods:
            self.current_value = np.nan
        elif len(self.closes) >= self.lookback_periods:
            pct_returns = np.array(self.closes)[-1:] / np.array(self.closes)[1:] - 1
...
class MACD(Feature):
    ...
    def _update(self, state: State) -> None:
        ...
        if len(self.closes) < self.lookback_periods:
            ...
            k = pd.Series(self.closes).ewm(span=12, adjust=False, min_periods=12).mean().values[-1]
            d = pd.Series(self.closes).ewm(span=26, adjust=False, min_periods=26).mean().values[-1]
